...
//   double Nd = double(N);
//   double Md = double(M);
//   double theta = 1.0;//R::runif(0.0, 2.0);
//   vector samp_weights = vector::Constant(N, 1.0/ Nd);
//   
//   matrix A = exp_cost.array()/exp_cost.lpNorm<1>();
//   vector log_r = mass_r.array().log();
...
//     u_hat = u_bar;
//     if(xi <= 0.5) {
//       double sub_u = (r_bar(pi(0)) - mass_r(pi(0))) / (eta * 8 * theta * Nd);
//       u_tilde(pi(0)) -=  sub_u; //note, eta here is reverse of paper my_eta = 1/their_eta
//       u_hat(pi(0)) -=  2 * Nd * theta * sub_u;
//       //v_tilde = v_tilde
//     } else {
//       //u_tilde = u_tilde
//       double sub_v = (l_bar(pi(0)) - mass_l(pi(0))) / (eta * 8 * theta * Md);
//       v_tilde(pi(0)) -= sub_v;
//       v_hat(pi(0)) -=  2 * Md * theta * sub_v;
//     }
//     
//     //step 4
//     argmin_f(mass_r, mass_l, A,
...
//     l = B.colwise().sum();
//     
//     int I = argmax_abs(mass_r, r);
//     int J = argmax_abs(mass_l, l);
//     
//     if( rho(mass_r(I), r(I)) >= rho(mass_l(J), l(J))) {
//       y_u = u;
//       y_u(I) += log_r(I) - std::log(r(I));
